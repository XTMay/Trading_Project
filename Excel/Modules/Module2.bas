...
Dim WT As Integer: WT = 3  ' IE wait time
 ...
 [i12] = Left(ActiveWorkbook.Name, InStr(1, ActiveWorkbook.Name, ".") - 1)
    ...
    Range("K12").FormulaR1C1 = "=R[18]C[12]" '_____延|率
...
Range(Columns(dc), Columns(dc + 100)).Clear
  ...
  sUrl(1, 2) = dc '_______Alex__鮮lex吳桑
  ...
  'sUrl(5, 2) = 76 'CC ___+__+__價+日期
  'sUrl(5, 3) = "____+__+__+__  5 / 9"紹  5 / 9"
...
Next i
[w25] = Mid(Cells(i, dc), InStr(1, Cells(i, dc), "values") + 7, 3) '___v用
...
'Call ushis '__20_______yahoo____yahoo已不給抓
'Call ConnectXMLHTTP(url)
...
dc = dc6
   ...
   If i < 999 Then
      ...
      [i1] = Mid(Cells(i + 4, dc), aa + 7, bb - aa + 7) '__期
      Cells(i + 4, dc - 1) = 1
...
Cells(i, dc - 1) = 1
[w25] = Mid(Cells(i, dc), InStr(1, Cells(i, dc), "are in ") + 7, 3) '___v用
Exit For
End If
Next i
...
cd$ = mkw([a2])
    ...
    'Call ConnectMarketWatch(url, Cells(2, dc ), 2)
     ...
     Call ConnectXMLHTTP(url)
         ...
         'If [zz1] = "" Or [zz1] Mod 3 = 0 Then ' "" means____, mod 3 from __m 收藏
         '       IE.Quit
         '       Set IE = Nothing
         '       Call DelIE
         'End If
...
bf2: If Cells(3, dc1) = "" Or Cells(3, dc1 + 9) = "" Or Cells(3, dc1 + 9 * 2) = "" Or Cells(3, dc1 + 9 * 3) = "" Then GoTo __式
...
[aa13] = [ae3]
For dc = dc1 To dc1 + 9 * 3 Step 9 ' 31 40 49 58 ________單位轉換
...
__: [y24] = [q9]9]
    ...
    TextToDisplay:="______- __Alex____"lex吳桑提供"
...
With ActiveSheet.Cells
    ...
    [h15].ShrinkToFit = False '____縮小
    [g44].ShrinkToFit = False '____縮小
    [g45].ShrinkToFit = False '____縮小
    Range(Cells(18, 10), Cells(25, 10)).ShrinkToFit = False '____縮小
    Range(Cells(37, 10), Cells(47, 10)).ShrinkToFit = False '____縮小
...
Public Function CCRUMBT(rng As Range) '______家晟桑
...
Public Function DCSV(url As String) '______家晟桑
    On Error Resume Next
    With CreateObject("WinHttp.WinHttpRequest.5.1")
        .Open "POST", url, False
        .Option(4) = 13056
...
Private Sub ushis() '__20_______yahoo____yahoo已不給抓
...
If i > t Then Cells(i, 21) = Cells(i - 1, 21) * Cells(i, 20) '____分割
