...
On Error Resume Next
    ...
    [a18] = "______________________________________"嚏B公式和明牌而未註明出處者即為小偷。"
    [a95] = "______________(Michael On)__"Michael On)所有"
    ...
    With Range(rsrange)
        ...
        [w30] = c.Offset(0, -1): [w31].Calculate: c.Offset(0, -1) = [w31] '____時間
        [w30] = c.Offset(1, -1): [w31].Calculate: c.Offset(1, -1) = [w31] '____時間
        End If
     
        Set c = .Find("___", LookIn:=xlValues)es)
        If Not c Is Nothing Then _
        [i1] = Year(Date) & "/" & Mid(c.Offset(-1, 0), 7, 5) '_____璈鬗
    ...
    If Not c Is Nothing Then
    [w30] = c.Offset(1, 1): [w31].Calculate: c.Offset(1, 1) = [w31]: [w32] = [w31] '___v日
    [w30] = c.Offset(1, 0): [w31].Calculate: c.Offset(1, 0) = [w31]: [w33] = [w31] '___坐
    End If
    Set c = Nothing
    ...
    If Cells(1000, dc).End(xlUp).Row < 10 Then '_____D合併
   ...
   ' End If
    ...
    If Cells(1000, dc + 11).End(xlUp).Row < 10 Then '_____D合併
   
        Range(Cells(2, dc + 11), Cells(1000, dc + 11 + 9)).Clear
        Application.StatusBar = "__________  3 / 11"  3 / 11"
        url = "http://" & urltype & "/z/zc/zcp/zcpa/zcpa0_" & cd$ & ".djhtm"
        Cells(1, dc + 11) = "3 / 11 _________ " & url " & url
        Call TWReport(url, dc + 11)
   ...
   ' End If
    ...
    If Cells(1000, dc + 22).End(xlUp).Row < 10 Then '_____D合併
   ...
   ' End If
    ...
    If Cells(1000, dc + 33).End(xlUp).Row < 10 Then '_____D合併
   
        Range(Cells(2, dc + 33), Cells(1000, dc + 33 + 9)).Clear
        Application.StatusBar = "__________  5 / 11"  5 / 11"
        url = "http://" & urltype & "/z/zc/zcp/zcpb/zcpb0_" & cd$ & ".djhtm"
        Cells(1, dc + 33) = "5 / 11 _________ " & url " & url
    ...
    For i = 0 To 3 '_________vlookup利vlookup
        For j = 1 To 200
    ...
    For k = 1 To 200 '_____@致化
        If Cells(k, dc) = "" And Cells(k + 1, dc) = "" Then Exit For
        If Left(Cells(k, dc).value, 6) = "______" Then Cells(k, dc).value = "______"輕蟋|後淨利"
        If Left(Cells(k, dc).value, 6) = "______" Then Cells(k, dc).value = "______"祤う捇v淨利"
    Next k
    
    For k = 1 To 200
    If Cells(k, dc + 22) = "" And Cells(k + 1, dc + 22) = "" Then Exit For
        If Left(Cells(k, dc + 22).value, 6) = "______" Then Cells(k, dc + 22).value = "______"輕蟋|後淨利"
        If Left(Cells(k, dc + 22).value, 6) = "______" Then Cells(k, dc + 22).value = "______"祤う捇v淨利"
    Next k
    
    '__利
    Application.StatusBar = "___  6 / 11"11"
    urltype1 = urltype
    If urltype = "justdata.yuanta.com.tw" Then urltype1 = "pscnetinvest.moneydj.com.tw" '___________〞爣o不一樣
    dc = dc6
    ...
    If mo$ = "01" Then mo$ = "02"
    [v35].NumberFormatLocal = "G/____"璁"
    If [v35] = 1 Then mo$ = "12" '_____撕L年
    dc = dc11
    ...
    Else
        Range("ea:ez").Clear     'OTC__價
        Call OTC(cd$, dc)
        If IsError([c12]) Then
            If Cells(5, dc).value = "" Then GoTo his '_______"史股價"
        Else
            If Cells(5, dc).value = "" And [c12] > 10 Then GoTo his '_______"史股價"
        End If
    End If
...
err:    [y24] = [q14]
        ...
        Address:="https://tw.stock.yahoo.com/q/ta?s=" & cd$, TextToDisplay:="___"圖"
    
        .Hyperlinks.Add Anchor:=[g15], _
        Address:="http://tw.stock.yahoo.com/q/q?s=" & cd$, TextToDisplay:="__"D"
    
        .Hyperlinks.Add Anchor:=[f15], _
        Address:="http://www.moneydj.com/KMDJ/Wiki/WikiSubjectList.aspx?a=TW." & cd$, TextToDisplay:="__""
        
        .Hyperlinks.Add Anchor:=[e15], _
        Address:="http://webretro.fortunengine.com.tw/stock/tool/diy_form.cfm", TextToDisplay:="___"器"
        ...
        Address:="http://mikeon88.imotor.com/forumdisplay.php?fid=2", TextToDisplay:="___2"2"
        
         .Hyperlinks.Add Anchor:=[j17], _
        Address:="http://mikeon88.freebbs.tw/forumdisplay.php?fid=52", TextToDisplay:="___1"1"
        ...
        Address:="https://mikeon88.666forum.com/f1-forum", TextToDisplay:="____"蚾"
    
        .Hyperlinks.Add Anchor:=[j16], _
        Address:="http://mikeon88.blogspot.com", TextToDisplay:="_______"部落格"
    End With
    
For m = 2 To 20
If Cells(m, dc7) <> "" Then Exit For
Next m
If m > 19 Then [b14] = "7 / 11 ____________"恁A其餘正常"

For m = 2 To 20
If Cells(m, dc6) <> "" Then Exit For
Next m
If m > 19 Then [b14] = "6 / 11 _____"股利"

For m = 2 To 20
If Cells(m, dc1 + 1) <> "" Then Exit For
Next m
If m > 19 Then [b14] = "1 / 11 _____ "捋 "

For m = 2 To 20
If Cells(m, dc2) <> "" Then Exit For
Next m
If m > 19 Then [b14] = "2 / 11 _____________"報，別再問了"

    If [a2] = "5306" Then
    [a1] = "__" & " (" & "5306" & ")" & "_" & "____"動產品"
    [a20] = "__""
    End If
    ...
    Range(Cells(2, 2), Cells(93, 10)).ShrinkToFit = True '___Y小
    [f12].ShrinkToFit = False '____縮小
    [i14].ShrinkToFit = False '____縮小
    [j16].ShrinkToFit = False '____縮小
    [e17].ShrinkToFit = False '____縮小
    [g60].ShrinkToFit = False '____縮小
    [g80].ShrinkToFit = False '____縮小
    [g81].ShrinkToFit = False '____縮小
    Range(Cells(20, 11), Cells(25, 10)).ShrinkToFit = False '____縮小
    Range(Cells(72, 11), Cells(81, 10)).ShrinkToFit = False '____縮小
    
    Range(Columns(dc2), Columns(dc2 + 150)).Select '___Y小
        With Selection
        .WrapText = False
        .ShrinkToFit = True
    End With
    
    Range(Cells(1, dc2), Cells(1, dc2 + 150)).ShrinkToFit = False '____縮小
    Cells(50, dc1 + 1).ShrinkToFit = False '____縮小
    Range(Cells(26, dc1 + 2), Cells(27, dc1 + 2)).ShrinkToFit = False '____縮小
    Range(Cells(3, dc8 + 9), Cells(5, dc8 + 9)).ShrinkToFit = False '____縮小
    Cells(19, dc11).ShrinkToFit = False '____縮小
    Cells(41, dc8 + 9).ShrinkToFit = False '____縮小
...
Public Sub TWReport(url, dc)
     ...
     Cells(2, dc) = Replace(Cells(2, dc), "__ ____ __", "")表", "")
...
Public Function ERRLOG(ST As String, EN As String)
    On Error Resume Next
    If Not ST = "" Then _
    ERRLOG = Replace("_" & ST, "$", "") & IIf(InStr(ST, ":") > 0, "", "____")常")
