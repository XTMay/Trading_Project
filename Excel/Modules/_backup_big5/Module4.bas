...
' mikeon_ 2007/5/7 _____ªº¥¨¶°



Sheets("__").Selectct
Application.Calculation = xlAutomatic '_____}¼sºÖ
[b11] = "__2_______..."h¾á«Ý..."
Application.Calculation = xlManual '____®}¼s
Call UnprotectSheet(ActiveSheet)
With Application '_____}¼sºÖ
         .MaxChange = 0.001    '____³t«×
...
Application.StatusBar = "___ 5 ____(______)  1 / 14"æ©u)  1 / 14"
...
Application.StatusBar = "___ 5 ____(______)  2 / 14"æ©u)  2 / 14"
...
If k < 200 Then
k = k - 1
Cells(k - 1, dc + 1 + 5 * i) = [aa30] '__­È
Cells(k, dc + 1 + 5 * i) = [aa31] '__²£
End If
...
For k = k To 500
 If Cells(k, dc) = "___" Then Exit For '___Á`¥«­È
Next k
Cells(k, dc - 1) = 1
...
'__+__+____ __Alex_______lex§d®á©M¤è®aÑÔ®á
...
'Call cnhis '__20_______yahoo____yahoo¤w¤£µ¹§ì
...
CN1:
 ...
 '__+_____1___10__ x _¨C10ªÑ°t x ¤¸
...
__: [y24] = [q6]6]
    ...
    [h13].Select
    Selection.Hyperlinks.Add Anchor:=Selection, _
    Address:="https://finance.yahoo.com/echarts?s=" & cd$ & "." & ex$ & "#symbol=" & cd$ & "." & ex$ & ";range=5y", TextToDisplay:="___"¹Ï"
    ...
    [f13].Select
    Selection.Hyperlinks.Add Anchor:=Selection, _
    Address:="http://www.aastocks.com/tc/cnhk/analysis/company-fundamental/company-profile?shsymbol=" & cd$, TextToDisplay:="__"Ð"
...
Next m
If m > 19 Then [b11] = "13 / 14 _____+__"§+ªÑ¤l"
...
If Cells(m, dc1) <> "" Then Exit For
Next m
If m > 19 Then [b11] = "1 / 14 _____________"³ø¡A§O¦A°Ý¤F"
...
With ActiveSheet.Cells
    ...
    Range(Cells(2, 2), Cells(52, 10)).ShrinkToFit = True '___Y¤p
    [f9].ShrinkToFit = False '____ÁY¤p
    [i12].ShrinkToFit = False '____ÁY¤p
    [e15].ShrinkToFit = False '____ÁY¤p
    [g42].ShrinkToFit = False '____ÁY¤p
    [g43].ShrinkToFit = False '____ÁY¤p
    Range(Cells(18, 11), Cells(23, 11)).ShrinkToFit = False '____ÁY¤p
    Range(Cells(36, 10), Cells(43, 10)).ShrinkToFit = False '____ÁY¤p
    ...
    Range(Cells(57, dc1), Cells(57, dc1 + 100)).ShrinkToFit = False '____ÁY¤p
...
Private Sub cnhis() '__20_______yahoo____yahoo¤w¤£µ¹§ì
