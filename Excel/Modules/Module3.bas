...
End With
    ...
    [a68] = "______________(Michael On)__"Michael On)所有"
...
For i = 3 To 90 '_________vlook__免vlook找錯
If Cells(i, dc) = "_______" Then Cells(i, dc) = "_______" '1及短期資金" '1
Next i
...
If Cells(5, dc + 1) <> "" And Not IsNumeric(Cells(5, dc + 1)) Then GoTo __式
If Cells(5, dc + 1) = 0 Then GoTo __式
...
'Call hkhis '__20_______yahoo____yahoo已不給抓
...
dc = dc9
   ...
   url = "https://finance.yahoo.com/quote/HKD" + [y25] + "=x?ltr=1" '__Alex_x吳
...
Cells(c - 2, dc + 11) = "__": Cells(c - 1, dc + 11) = "__"鉹"
i = c
...
cx5: If Cells(4, dc7) = "" Then GoTo __式
...
Selection.Hyperlinks.Add Anchor:=Selection, _
    Address:="https://finance.yahoo.com/echarts?s=" & cd$ & ".HK#symbol=" & cd$ & ".HK;range=5y", TextToDisplay:="___"圖"
...
Selection.Hyperlinks.Add Anchor:=Selection, _
    ...
    [e13].Select
Selection.Hyperlinks.Add Anchor:=Selection, _
    Address:="https://www.aastocks.com/tc/stocks/analysis/company-fundamental/company-profile?symbol=" & [a2], TextToDisplay:="__""
...
Next m
If m > 19 Then [b11] = "7 / 9 _______"史股價"
...
Next m
If m > 19 Then [b11] = "4 / 9 _______"資產表"

For m = 2 To 20
If Cells(m, dc3) <> "" Then Exit For
Next m
If m > 19 Then [b11] = "3 / 9 _______"損益表"

For m = 2 To 20
If Cells(m, dc2) <> "" Then Exit For
Next m
If m > 19 Then [b11] = "2 / 9 ________"~資產表"

For m = 2 To 20
If Cells(m, dc1) <> "" Then Exit For
Next m
If m > 19 Then [b11] = "1 / 9 ________"~損益表"
...
With ActiveSheet.Cells
    ...
    Range(Cells(2, 2), Cells(66, 10)).ShrinkToFit = True '___Y小
    [f9].ShrinkToFit = False '____縮小
    [i12].ShrinkToFit = False '____縮小
    [e15].ShrinkToFit = False '____縮小
    [g55].ShrinkToFit = False '____縮小
    [g56].ShrinkToFit = False '____縮小
    Range(Cells(18, 11), Cells(23, 11)).ShrinkToFit = False '____縮小
    Range(Cells(43, 10), Cells(56, 10)).ShrinkToFit = False '____縮小
...
Private Sub hkhis() '__20_______yahoo____yahoo已不給抓
