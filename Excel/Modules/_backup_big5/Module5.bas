...
With Application
    '.Calculation = xlManual '____®}¼s
    .Calculation = xlAutomatic '_____}¼sºÖ
...
PL_Q:
        ...
        If Len(Cells(3, dc)) = 0 Then
            ...
            ElseIf no_retry_report >= 4 Then
                ...
                MsgBox "_____________(" & url & "), _____", vbOKOnlyy«á¦A¸Õ", vbOKOnly
...
PL_Y:
   ...
   [af64] = Cells(i, dc + 18)
   [ab35] = Cells(i, dc + 18)
   [ae25] = Mid([af64], InStr(1, [af64], "values") + 7, 3) '___v¥Î
...
Application.StatusBar = "Historical stock price  5 / 9" '__Alex_______M¤è®aÑÔ®á
...
'Call glhis '__20_______yahoo____yahoo¤w¤£µ¹§ì
...
dc = dc6
 ...
 '   Delete_Pictures '__Alex_x§d
   ...
   Next i
   [i1] = Right(Cells(i, dc), 8) '__´Á
   Cells(i, dc - 1) = 1
   If Cells(i + 1, dc) <> "" Then
   [q6] = Cells(i + 1, dc) '__»ù
   Cells(i + 1, dc - 1) = 1
   Else: [q6] = Cells(i + 2, dc)
   Cells(i + 2, dc - 1) = 1
   End If
   [ac25] = UCase(Right(Trim([q6]), 3)) '____¹ô§O
...
With ActiveSheet.Cells
    ...
    [h14].ShrinkToFit = False '____ÁY¤p
    [g44].ShrinkToFit = False '____ÁY¤p
    [g45].ShrinkToFit = False '____ÁY¤p
    Range(Cells(18, 11), Cells(25, 11)).ShrinkToFit = False '____ÁY¤p
    Range(Cells(37, 11), Cells(47, 11)).ShrinkToFit = False '____ÁY¤p
    ...
    With Selection
        .HorizontalAlignment = xlCenter '___m¤¤
...
Public Function CCRUMBT_BAK(rng As Range) '______®aÑÔ®á
...
Public Function DCSV_BAK(url As String) '______®aÑÔ®á
    On Error Resume Next
    With CreateObject("WinHttp.WinHttpRequest.5.1")
        .Open "POST", url, False
        .Option(4) = 13056
...
Private Sub glhis() '__20_______yahoo____yahoo¤w¤£µ¹§ì
Dim myurl(2)
myday = DateDiff("s", "1/1/1970", Now()) ' unix timestamp
